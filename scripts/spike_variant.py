...
from spike_pathogenic import (
    _CHROM_ORDER,
    _open,
    load_exome,  # noqa: E402
    mc_consequence,
    parse_info,
    spiked_line,
)
...
def _pick_template(records, zyg):
    ...
    for f in records:
        ...
        fmt = f[8].split(":")
        if (
            "GT" not in fmt or "F1R2" not in fmt
        ):  # require a full DRAGEN FORMAT template
            continue
        smp = f[9].split(":")
...
def main():
    ...
    rec = {
        ...
        "clnrevstat": cv.get("clnrevstat")
        or ("criteria provided, single submitter" if not cv else ""),
        "clndn": _clean(cv.get("clndn") or a.disease or ""),
    }
    src = (
        "exact ClinVar match"
        if cv
        else "coord planted; synthetic CLNSIG (not in this ClinVar)"
    )
    print(
        f"  spike {a.gene} {a.chrom}:{a.pos} {a.ref}>{a.alt} [{rec['csq']}, {rec['clnsig']}] — {src}",
        file=sys.stderr,
    )
    ...
    if a.realistic:
        ...
        if spike is None:
            print(
                "  WARN: no full-FORMAT background call of matching zygosity to borrow — "
                "falling back to the minimal (tell-bearing) record",
                file=sys.stderr,
            )
        else:
            used_realistic = True
            print(
                "  realistic: borrowed a real background call's INFO/FORMAT; no spike markers "
    ...
    comment = (
        "##comment=SYNTHETIC: real 1000G background with ONE exact ClinVar/phenopacket "
    ...
    with open(a.out, "w") as out:
        ...
        for f in all_rows:
            out.write("\t".join(f) + "\n")
    print(
        f"Wrote {a.out}: {len(records)} background + 1 spiked = {len(all_rows)} variants "
