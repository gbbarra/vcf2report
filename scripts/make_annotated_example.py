...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(
        description="Bake store annotations into a SnpEff VCF -> example."
    )
    ap.add_argument("--vcf", required=True, help="SnpEff-annotated (tell-free) VCF")
    ap.add_argument(
        "--out", required=True, help="output annotated example VCF (.vcf.gz)"
    )
    ap.add_argument(
        "--keep", required=True, help="planted variant chrom:pos:ref:alt (always kept)"
    )
    ap.add_argument(
        "--max-background",
        type=int,
    ...
    if not plant_found:
        print(
            f"WARNING: planted variant {args.keep} not found in {args.vcf}",
            file=sys.stderr,
        )
    ...
    if primed == 0:
        print(
            "WARNING: 0 gnomAD hits — are the stores present? (check_stores.py)",
            file=sys.stderr,
        )
    ...
    for v, a in zip(variants, anns):
        add_alphamissense(
            v, a
        )  # AlphaMissense for all kept (per-variant tabix/parquet)
    ...
    for h in header:
        if h.startswith("#CHROM"):
            out_header += [
                x
                for x in _HEADER_LINES
                if not any(x.split(",")[0] in hh for hh in header)
            ]
        out_header.append(h)
    ...
    with gzip.open(args.out, "wt") as out:
        ...
        out.write("\n".join(out_data) + "\n")
    print(
        f"wrote {len(out_data)} variants ({baked} store-annotated, plant {'kept' if plant_found else 'MISSING'}) -> {args.out}",
        file=sys.stderr,
    )
    return 0
