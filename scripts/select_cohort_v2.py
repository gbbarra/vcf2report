...
used_genes = {
    r["gene"].upper()
    for r in csv.DictReader(open("data/synthetic_cohort/cohort.tsv"), delimiter="\t")
}
used_coords = {
    (norm(r["chrom"]), str(r["pos"]), r["ref"].upper(), r["alt"].upper())
    for r in csv.DictReader(open("data/synthetic_cohort/cohort.tsv"), delimiter="\t")
}
...
def classify(hp, ref, alt):
    ...
    if re.search(r"del|dup|ins", hp):  # in-frame if len diff %3==0
        return (
            "inframe_indel"
            if abs(len(ref) - len(alt)) % 3 == 0 and len(ref) != len(alt)
            else "frameshift_variant"
        )
    if re.search(r"p\.[A-Z][a-z][a-z]\d+[A-Z][a-z][a-z]", hp):
        return "missense_variant"
    if len(ref) == 1 and len(alt) == 1:
        return "missense_variant"  # SNV c.-level, assume missense-ish
    return "other"
...
for fp in glob.glob(f"{SP}/pp/**/*.json", recursive=True):
    ...
    vs = [
        ...
        if all(v.get(k) for k in ("chrom", "pos", "ref", "alt"))
    ]
    if not vs:
        continue
...
for c, n in Counter(v["cons"] for v in cases.values()).most_common():
    print(f"  {n:4}  {c}")
...
for v in cases.values():
    bygroup.setdefault(v["cons"], []).append(v)
sel = []
...
for c, n in Counter(v["cons"] for v in sel).most_common():
    print(f"  {n:3}  {c}")
print(
    f"missense+inframe (VUS-producing): {sum(1 for v in sel if v['cons'] in ('missense_variant', 'inframe_indel'))}/100 (v1 era 45)"
)
...
for i, v in enumerate(sel):
    ...
    pz = next(
        (
            x.get("zygosity")
            for x in vs
            if (norm2(x["chrom"]), str(x["pos"]), x["ref"].upper(), x["alt"].upper())
            == pkey
        ),
        "het",
    )
    ...
    if (pz or "").startswith("hom"):
        plan[sid] = {"mode": "hom"}
    elif others:
        o = others[0]
        plan[sid] = {
            "mode": "compound_het",
            "chrom": "chr" + norm2(o["chrom"]),
            "pos2": int(o["pos"]),
            "ref2": o["ref"].upper(),
            "alt2": o["alt"].upper(),
            "zyg2": o.get("zygosity", "het"),
        }
    else:
...
print(
    f"    backgrounds distintos dos usados: {len(set(r[1] for r in rows) - set(open(f'{SP}/used_bg.txt').read().split()))}/100"
)
print("    primeiros 3:")
for r in rows[:3]:
    print(f"      {r[0]} {r[1]} {r[2]} {r[3]}:{r[4]} {r[7]}")
