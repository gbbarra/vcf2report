...
def main(argv=None) -> int:
    ...
    prefix = (
        ...
        if args.out_prefix
        else Path(args.phenopacket).with_suffix("")
    )
    ...
    if data.get("skipped_variants"):
        print(
            f"WARNING: {data['skipped_variants']} variant(s) skipped (HGVS-only, "
            f"no VCF coordinates) — resolve HGVS to coordinates to include them."
        )
    print(f"wrote {vcf} and {hpo}")
    print(
        "NOTE: this VCF has no molecular consequence — annotate it "
