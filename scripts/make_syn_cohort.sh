...
tail -n +2 "$COHORT" | while IFS=$'\t' read -r syn sample gene chrom pos ref alt cons disease hpo; do
  ...
  # S3 throughput to this network fluctuates (fast bursts + throttled windows). Abort a stall
  # (<1 MB/s for 25 s) and retry on a fresh connection, resuming (-C -) from the partial. If it
  # still can't pull the file, SKIP this case and move on — the run is resumable, so a later re-run
  # fills the gap during a fast S3 window — rather than hanging or aborting the whole cohort.
  if ! curl -fL --retry 6 --retry-delay 2 --speed-limit 1000000 --speed-time 25 \
    ...
    echo "  WARN: $sample download too slow / failed — SKIPPING (re-run later to retry)"
    rm -rf "$work"; continue
  fi
