...
from vcf2report.concordance import (
    _annotation_from_frozen,
    classify_entry,  # noqa: E402
...
def analyse() -> dict:
    ...
    for r in res:
        ups = [
            w for w in (r.get("would_change_with") or []) if w.get("direction") == "up"
        ]
        if not ups:
    ...
    for e in load_panel():
        ...
        ann = replace(
            _annotation_from_frozen(e, True),
            hpo_match_score=1.0,
...
def main() -> int:
    ...
    print(
        "  Every one is ONE line of evidence short — none is misclassified toward benign.\n"
    )
    ...
    if r["by_strength"].get("unreachable with one line"):
        print(
            f"    {r['by_strength']['unreachable with one line']:>3}  unreachable with one line"
        )
