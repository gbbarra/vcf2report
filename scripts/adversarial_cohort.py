...
def _analyze(vcf, hpo, gene):
    ...
    return dict(
        engine_plp=engine_plp,
        pheno_primary=pheno_primary,
        clinvar_flag=clinvar_flag,
        n_plp=n_plp,
        surfaced=engine_plp or pheno_primary or clinvar_flag,
...
def _vcf_for(syn):
    return (
        (DIR / "annotated" / f"{syn}.annotated.vcf.gz")
        if ANNOTATED
...
def _one(args):
    ...
    return {
        ...
        "engine_plp": r["engine_plp"],
        "pheno_primary": r["pheno_primary"],
        "clinvar_flag": r["clinvar_flag"],
        "n_plp": r["n_plp"],
        "decoy_pheno_primary": d["pheno_primary"],
        "decoy_surfaced": d["surfaced"],
    }
...
def main():
    ...
    ap.add_argument(
        "--limit", type=int, default=0, help="only the first N cases (smoke test)"
    )
    ...
    if missing:
        sys.exit(
            f"ERROR: {len(missing)} VCF(s) not found (e.g. {_vcf_for(missing[0])}). "
            + ("Run: bash scripts/annotate_syn_cohort.sh" if ANNOTATED else "")
        )
    label = (
        "ANNOTATED (SnpEff MANE)"
        if ANNOTATED
    ...
    with Pool(args.jobs) as pool:  # conservative for a laptop's RAM
        ...
        for k, res in enumerate(pool.imap_unordered(_one, tasks), 1):
            results.append(res)
            print(
                f"  [{k}/{len(tasks)}] {res['syn']} {res['gene']} "
                f"{'ok' if 'error' not in res else 'ERR'}",
                flush=True,
            )

    out = DIR / (
        "adversarial_results_annotated.tsv" if ANNOTATED else "adversarial_results.tsv"
    )
    cols = [
        "syn",
        "sample",
    ...
    print(
        f"ENGINE-ONLY (no ClinVar flag): {pct('engine_only')}   <- anti-circular metric"
    )
    print(f"  via engine P/LP tier:      {pct('engine_plp')}")
    print(f"  via phenotype-primary:     {pct('pheno_primary')}")
    print(
        f"surfaced ONLY by ClinVar flag: "
        f"{sum(r['clinvar_flag'] and not r['engine_only'] for r in good)}/{n}"
    )
    print(
        f"DECOY pheno-primary (wrong HPO still routes gene): {pct('decoy_pheno_primary')}   "
        f"<- should be LOW (phenotype specificity)"
    )
    print(
        f"over-call — P/LP calls per case (~100k background): "
        f"median {nplp[n // 2]} / max {nplp[-1]} / min {nplp[0]}"
    )
    if not ANNOTATED:
        print(
            "  ^ ARTIFACT: the raw background has no consequence, so it CANNOT be called P/LP.\n"
