...
def _zygosity_of(descriptor: dict) -> str | None:
    ...
    return _ZYGOSITY.get(state.get("id")) or _ZYGOSITY.get(
        (state.get("label") or "").lower()
    )
...
def _variant_from_descriptor(descriptor: dict) -> dict | None:
    ...
    if (
        not rec.get("chrom")
        or rec.get("pos") in (None, "")
        or not rec.get("ref")
        or not rec.get("alt")
    ):
        return None
    exprs = {
        e.get("syntax"): e.get("value") for e in descriptor.get("expressions", []) or []
    }
