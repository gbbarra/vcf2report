...
def _request(
    req: urllib.request.Request, timeout: float, retries: int
) -> Optional[dict]:
...
def post_json(
    url: str, payload: dict, timeout: float = 15.0, retries: int = 2
) -> Optional[dict]:
...
def get_json(
    url: str, params: dict, timeout: float = 15.0, retries: int = 2
) -> Optional[dict]:
