...
def run_report(
    ...
    """
    report = run_pipeline(
        vcf_path, hpo_terms=hpo_terms or [], sample_id=sample_id or None
    )
    ...
    return {
        ...
        "tiers": [
            {
                "gene": c.variant.gene,
                "variant": c.variant.hgvs_p or c.variant.key,
                "tier": c.tier,
...
def _annotate_vcf(vcf_path: str, reference: str = "", out_dir: str = "") -> dict:
    ...
    if not missing_tools:
        ...
        annotated_out = out / (
            Path(vcf_path).stem.replace(".vcf", "") + ".annotated.vcf.gz"
        )
        script = str(config.REPO_ROOT / "scripts" / "annotate_vcf.sh")
        cmd = ["bash", script, vcf_path, str(annotated_out)] + (
    ...
    return {
        ...
        "steps": [
            "not annotated and (tools or reference) unavailable; classification "
...
@mcp.tool()
def explore_evidence_sources(
    results_json: str, view: str = "clinvar", include_not_met: bool = False
) -> dict:
    ...
    if v == "missense":
        return {
            "view": v,
            "findings": missense_evidence(data, met_only=not include_not_met),
        }
    if v != "clinvar":
