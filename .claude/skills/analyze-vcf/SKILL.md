...
- `cd` into the `vcf2report` repo if you know where it is. If not, ask the user, or
  offer to clone it (Step 1). Confirm `pyproject.toml` and `scripts/run_headless.py`
...
3. If it's not installed, guide the user (or run it yourself if you're in the repo):
   ```bash
   git clone https://github.com/gbbarra/vcf2report.git   # skip if already cloned
   cd vcf2report
   python -m pip install -e .          # only runtime dep is jinja2 — demo databases are bundled
   ```
   ⚠️ Tell them **not** to clone *inside* an existing `vcf2report/` folder (it nests).
...
2. Build a self-contained HTML report from it and publish it with the **Artifact**
   tool, using `references/report_template.html` as the exact style + structure.
