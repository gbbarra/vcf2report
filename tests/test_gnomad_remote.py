...
def test_excluded_grpmax_falls_back_to_global(monkeypatch):
    rec = _Rec(
        ...
        {
            "grpmax": ("fin",),
            "AF_grpmax": (0.02,),  # fin excluded from popmax
            "AF": (0.001,),
            "AC": (30,),
            "AN": (30000,),
            "nhomalt": (0,),
...
def test_takes_higher_of_exome_genome(monkeypatch):
    ex = _Rec(
        100, "G", ("T",), {"AF": (0.001,), "AC": (10,), "AN": (10000,), "nhomalt": (0,)}
    )
    ge = _Rec(
        100, "G", ("T",), {"AF": (0.004,), "AC": (8,), "AN": (2000,), "nhomalt": (0,)}
    )
...
def test_not_runnable_returns_none(monkeypatch):
    ...
    monkeypatch.setattr(
        gnomad_remote, "_open", lambda kind, chrom: None
    )  # nothing opens
