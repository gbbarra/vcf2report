...
from vcf2report.report.render import (HOIST_MIN_VARIANTS, _uninformative,
                                      render_markdown, run_constant_criteria)


N = HOIST_MIN_VARIANTS          # enough variants for the hoist to engage


def _cr(code, met=False, evidence=None, reasoning="because", citation=None, applies=True):
    return CriterionResult(code, f"{code} name", "supporting", applies=applies, met=met,
...
def _cls(gene, criteria, tier="Uncertain Significance (VUS)"):
    return Classification(
        ...
        annotation=Annotation(), criteria=criteria, tier=tier, rule_path="x => y")


def _report(classifications):
    return ReportModel(sample_id="S", hpo_terms=[], qc=QCSummary(),
...
def test_a_criterion_that_never_fires_is_stated_once():
    r = _report([_cls(f"G{i}", [_cr("PS2", reasoning="Requires parental (trio) data")])
                 for i in range(N)])
...
def test_a_never_fired_criterion_with_evidence_is_still_hoisted():
    ...
    r = _report([_cls(f"G{i}", [_cr("PM1", evidence={"window": 7, "cutoff": 3},
...
def test_mixed_reasons_report_the_dominant_one_and_admit_the_rest():
    ...
    crits = [[_cr("PM1", reasoning="residue index unavailable" if i < N - 1 else "not missense")]
             for i in range(N)]
...
def test_the_rendered_report_declares_what_it_omitted():
    ...
    md = render_markdown(_report([
        _cls(f"G{i}", [_cr("PS2", reasoning="Requires parental (trio) data"),
                       _cr("PP3", met=True, evidence={"am": 0.9})]) for i in range(N)]))
    ...
    assert md.count("| **PS2** |") == 0, "the hoisted criterion is still printed per variant"
...
def test_per_variant_omissions_are_counted_not_silent():
    ...
    md = render_markdown(_report(
        [_cls("A", [_cr("BP7", evidence={}, reasoning="not synonymous"),
                    _cr("PP3", met=True, evidence={"am": 0.9})])]
        + [_cls(f"G{i}", [_cr("BP7", met=True, evidence={"x": 1}),
                          _cr("PP3", met=True, evidence={"am": 0.8})]) for i in range(N)]))
...
def test_a_small_report_is_never_trimmed():
    ...
    """
    small = [_cls("A", [_cr("PS2", reasoning="Requires parental (trio) data"),
