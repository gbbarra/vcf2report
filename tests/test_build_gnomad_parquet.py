...
def test_resume_skips_only_on_scope_match(tmp_path):
    ...
    (part / "_scope.json").write_text(
        json.dumps({"region": None, "bed": None, "preset": "exomes"})
    )
    ...
    assert (
        bgp.build_chrom(
            "21", url, fields, "/nonexistent", out, None, None, None, "exomes"
        )
        == 1
    )
    ...
    assert (
        bgp.build_chrom(
            "21", url, fields, "/nonexistent", out, None, None, None, "joint"
        )
        == 0
    )
...
def test_core_columns_still_present():
    for preset in ("joint", "exomes"):
        f = _fields(preset)
        assert {"af", "af_grpmax", "ac", "an", "nhomalt", "faf95", "grpmax_pop"} <= set(
            f
        )
...
def test_bcftools_fmt_and_copy_sql_reference_the_pop_fields():
    ...
    sql = bgp._copy_sql(
        pathlib.Path("/tmp/x.tsv"), pathlib.Path("/tmp/o.parquet"), cols
    )
