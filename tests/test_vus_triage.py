...
def _c(
    gene, *, tier=_VUS, am=None, hpo=0.9, clinvar=None, lof_intol=False, hp="p.Gly97Arg"
):
...
def test_phenotype_gate_excludes_incidental_vus():
    ...
    assert (
        probable_pathogenic_vus([_c("INCID", am=0.99, hpo=0.0, clinvar="Pathogenic")])
        == []
    )
...
def test_only_vus_tier_considered():
    assert (
        probable_pathogenic_vus([_c("PLP", tier="Pathogenic", am=0.99, hpo=1.0)]) == []
    )


def test_ranked_by_evidence_weight():
    strong = _c(
        "STRONG", am=0.97, hpo=1.0, clinvar="Pathogenic"
    )  # AM3 + hpo2 + clinvar2 = 7
    ...
    assert genes == [
        "STRONG",
        "MID",
    ]  # weak gated out (molecular < 2), strong ranks above mid


def test_conflicting_clinvar_is_a_signal():
    r = probable_pathogenic_vus(
        [_c("CONF", am=0.6, hpo=1.0, clinvar="Conflicting interpretations")]
    )
