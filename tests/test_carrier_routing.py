...
def test_carrier_does_not_displace_the_true_diagnosis(moi):
    ...
    moi({"POGZ": ["AD"], "PMM2": ["AR"]})
    primary, _sec, other = split_findings(
        [_c("POGZ", tier="Likely Pathogenic"), _c("PMM2")]
    )
...
def test_recessive_sf_carrier_is_not_an_actionable_secondary(moi):
    ...
    _pri, secondary, other = split_findings(
        [_c("ATP7B", hpo=0.0), _c("MUTYH", hpo=0.0)]
    )
    assert secondary == []
...
def test_vus_carrier_is_not_flagged(moi):
    ...
    moi({"RECGENE": ["AR"]})
    assert not is_unconfirmed_ar_carrier(
        _c("RECGENE", tier="Uncertain Significance (VUS)"), []
    )
