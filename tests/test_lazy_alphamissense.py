...
def _v(**kw):
    base = dict(
        chrom="1", pos=100, ref="A", alt="T", gene="G", consequence="missense_variant"
    )
    base.update(kw)
    return Variant(**base)
...
def test_deferred_still_reads_am_from_vcf_info(monkeypatch):
    def boom(v):
        raise AssertionError(
            "client must NOT be called when INFO already has the score"
        )
...
def test_pipeline_queries_am_only_for_candidates(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda v: (
            calls.append(v.key),
            {"am_pathogenicity": None, "am_class": None, "_source": "mock"},
        )[1],
    )
...
def test_lazy_tiers_equal_eager_tiers(monkeypatch):
    ...
    annotated = [
        (v, annotate_variant(v, [], build_trusted=True, with_alphamissense=True))
        for v in kept
    ]
    ...
    assert any(
        ("PP3" in c.met_codes) or ("BP4" in c.met_codes)
...
def test_default_annotate_variant_queries_client(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda v: (
            calls.append(v.key),
            {"am_pathogenicity": 0.88, "am_class": "ambiguous", "_source": "mock"},
        )[1],
    )
    v = _v()
