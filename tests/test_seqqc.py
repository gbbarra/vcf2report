...
def _v(ref="A", alt="G", depth=30, gq=50, zyg="het", pos=1):
    return Variant(
        chrom="1", pos=pos, ref=ref, alt=alt, depth=depth, gq=gq, zygosity=zyg
    )
...
def test_depth_and_gq_stats():
    vs = [
        _v(depth=d, gq=g, pos=i)
        for i, (d, g) in enumerate([(5, 10), (15, 25), (30, 60), (40, 99)])
    ]
    q = seqqc.estimate(vs)
...
def test_het_hom_ratio():
    q = seqqc.estimate(
        [_v(zyg="het", pos=1), _v(zyg="het", pos=2), _v(zyg="hom", pos=3)]
    )
...
def test_multiallelic_by_site():
    ...
    vs[0].n_alts = vs[1].n_alts = (
        2  # the two records at pos 10 share a multiallelic site
    )
    q = seqqc.estimate(vs)
