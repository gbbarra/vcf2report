...
def test_qc_dropped_clinvar_pathogenic_is_named_not_deleted(tmp_path, monkeypatch):
    ...
    assert "SCN1A" not in [
        c.variant.gene for c in r.classifications
    ]  # still dropped...
...
def test_no_live_artifact_tag_rests_on_missing_data():
    ...
    for f in (
        "data/example/SYN-070.RBSN.annotated.vcf.gz",
        "data/example/SYN-004.NIPBL.annotated.vcf.gz",
    ):
        for c in run_pipeline(f).classifications:
            if is_hom_absent_artifact(c):
                assert c.annotation.gnomad_an, (
                    f"{c.variant.gene}: tagged absent with AN=0"
                )
...
def test_phenotype_claims_survive_when_a_phenotype_was_supplied():
    ...
    r = run_pipeline(
        "data/example/SYN-001.synthetic.vcf.gz",
...
def test_per_allele_insilico_scores_are_not_shared_between_alleles(tmp_path):
    ...
    scores = {
        v.alt: from_vcf.extract(v).get("am_pathogenicity") for v in parse_vcf(p)[0]
    }
...
def test_snpeff_ann_is_still_parsed_positionally():
    ...
    assert (
        r["gene"] == "RBSN"
        and r["hgvs_c"] == "c.289G>C"
        and r["hgvs_p"] == "p.Gly97Arg"
    )
...
@pytest.mark.parametrize(
    "term", ["transcript_ablation", "exon_loss_variant", "protein_altering_variant"]
)
...
def test_eff_exon_deleted_survives_end_to_end():
    r = annparse.parse_snpeff_eff(
        "EXON_DELETED(HIGH||||NIPBL|protein_coding|CODING|NM_133433.4|5|1)"
    )
...
def test_known_pathogenic_splice_region_still_reaches_the_shortlist():
    ...
    known, _f = filter_variants(
        [(v, Annotation(clinvar_significance="Pathogenic", gnomad_af=0.0))]
    )
...
def test_every_demoted_hom_absent_variant_is_explained():
    ...
    assert "Verify the genotype" in " ".join(
        summarize(_report([vus]))
    )  # ...but explained
...
def test_a_store_vouched_absence_is_not_mistaken_for_missing_coverage():
    ...
    uncovered = Classification(
        variant=v,
        annotation=Annotation(gnomad_af=0.0, gnomad_ac=0, gnomad_an=0),
        criteria=[],
        tier="Likely Pathogenic",
        rule_path="",
    )
    ...
    surveyed = Classification(
        variant=v,
        annotation=Annotation(gnomad_af=0.0, gnomad_ac=0, gnomad_an=125000),
        criteria=[],
        tier="Likely Pathogenic",
        rule_path="",
    )
...
def test_a_filter_dropped_known_pathogenic_is_named(tmp_path):
    ...
    assert any(
        "KCNQ2" in s and "FILTER=LowQual" in s for s in r.qc.qc_rescued
    )  # ...but named
    from vcf2report.report.render import _render_markdown_builtin, render_markdown
...
def test_pm2_does_not_claim_local_cohort_was_consulted_when_it_was_not():
    ...
    unchecked = pm2(
        v, Annotation(gnomad_af=0.0, gnomad_an=152000, local_cohort_af=None)
    )
    ...
    assert (
        ...
        and checked.evidence["local_cohort_checked"] is True
    )

    common_in_brazil = pm2(
        v, Annotation(gnomad_af=0.0, gnomad_an=152000, local_cohort_af=0.032)
    )
